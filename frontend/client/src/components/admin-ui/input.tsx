    ...
    return (
      <div className="flex flex-col gap-2">
        {label && (
          <label
            htmlFor={inputId}
            className="text-xs font-semibold text-[var(--color-ink-3)] tracking-wide"
          >
            {label}
          </label>
        )}
        ...
        <div className="relative">
          {icon && (
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-ink-3)] pointer-events-none">
              {icon}
            </div>
          )}
          <input
            ...
            className={cn(
              ...
              // States: outline for focus (never animate the ring)
              "focus-visible:outline-none focus-visible:border-[var(--color-accent)] focus-visible:bg-[var(--color-surface)]",
              "focus-visible:shadow-[0_0_0_3px_var(--color-focus)]",
              // Hover
              "hover:border-[var(--color-neutral)] hover:bg-[var(--color-surface)]",
              // Disabled
              "disabled:opacity-55 disabled:cursor-not-allowed disabled:bg-[var(--color-surface-2)] disabled:border-[var(--color-rule)]",
              // Error
